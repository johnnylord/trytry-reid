...
__all__ = [ "ReidImageDataset", "ReidPoseDataset" ]
...
class PoseDataset:
    """Base class for all reid image pose dataset

    PoseDataset can be divided into three categories:
        1. train
        2. query
        3. gallery

    `query` and `gallery` dataset are from the same pid space. `train` has its
    own pid space. PoseDataset will organize these three dataset, and pack them
    into a single dataset.
    ...
    Attributes:
        ...
        query (list of tuple):  reid dataset from query source
        gallery (list of tuple):reid dataset from gallery source
        img_transform (function):   tranformation function for rgb image
        pose_transform (function):  tranformation function for pose image
        mode (str): data source
    """
    def __init__(self, train, query, gallery,
                img_transform=None,
    ...
    def __add__(self, other):
        """Combine training datasets together"""
        train = copy.deepcopy(self.train)
        for img_path, pose_path, pid, camid in other.train:
            pid += self.num_train_pids
            camid += self.num_train_cams
            train.append((img_path, pose_path, pid, camid))

        return ReidPoseDataset(train,
                            self.query,
                            self.gallery,
    ...
    def _get_num_pids(self, data):
        return len(set([ pid for _, _, pid, _ in data ]))

    def _get_num_cams(self, data):
        return len(set([ camid for _, _, _, camid in data ]))
...
class ReidPoseDataset(PoseDataset):
    """Base class for reid image & pose dataset"""

    def __init__(self, train, query, gallery, **kwargs):
        super().__init__(train, query, gallery, **kwargs)

    def __getitem__(self, idx):
        img_path, pose_path, pid, camid = self.data[idx]
        ...
        return img, pose, pid, camid, img_path, pose_path

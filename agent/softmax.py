...
__all__ = [ "SoftmaxAgent" ]

class SoftmaxAgent:
    """Train ReID model with CrossEntropy Loss"""
    def __init__(self, config):
        ...
        self.optimizer = optim.Adam(self.model.parameters(),
                                    ...
                                    weight_decay=1e-4)
        self.schedular = lr_scheduler.StepLR(self.optimizer,
    ...
    def train_one_epoch(self):
        ...
        for batch_idx, (imgs, pids, _, _) in enumerate(self.train_loader):
            ...
            _, labels = self.model(imgs)
            loss = self.criterian(labels, pids)
        ...
        self.writer.add_scalar("Epoch Loss", epoch_loss, self.current_epoch)
        self.writer.add_scalar("Epoch Acc", epoch_acc, self.current_epoch)
        print("Epoch {}:{}, Loss: {:.5f}, Acc:{:.2f}".format(
    ...
    def evaluate(self):
        ...
        query_features = np.concatenate(query_features) # (N, 128)
        query_pids = np.concatenate(query_pids)         # (N,)
        query_camids = np.concatenate(query_camids)     # (N,)
        ...
        gallery_features = np.concatenate(gallery_features) # (M, 128)
        gallery_pids = np.concatenate(gallery_pids)         # (M,)
        gallery_camids = np.concatenate(gallery_camids)     # (M,)

...
__all__ = [ "TripletAgent" ]

class TripletAgent:
    """Train ReID model with Triplet Loss"""
    def __init__(self, config):
        ...
        self.optimizer = optim.Adam(self.model.parameters(), lr=config['optimizer']['lr'], weight_decay=1e-4)
    ...
    def train_one_epoch(self):
        ...
        for batch_idx, (imgs, pids, _, _) in enumerate(self.train_loader):
            ...
            running_triplet_loss += loss.item() * count
            running_triplet_count += count

        # Export training result
        epoch_loss = running_triplet_loss / running_triplet_count
        self.writer.add_scalar("Epoch Train Loss", epoch_loss, self.current_epoch)
        print("Epoch {}:{}, Loss: {:.5f}, Avg Count: {}".format(
                self.current_epoch, self.config['train']['n_epochs'],
                epoch_loss, running_triplet_count//len(self.train_loader)))
